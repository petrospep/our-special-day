...
Deno.serve(async (req) => {
  ...
  const [{ count: rsvpCount, error: rsvpError }, { count: songCount, error: songError }] =
    await Promise.all([
      auth.supabaseAdmin
        .from("rsvp_responses")
  ...
  if (rsvpError || songError) {
    console.error(rsvpError ?? songError);

    return withCors(json({ ok: false, error: "invite_delete_failed" }, 500));
  }

  if ((rsvpCount ?? 0) > 0 || (songCount ?? 0) > 0) {
